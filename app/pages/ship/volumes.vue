...
const volumesApi = useQuadletVolumes()

const volumes = ref<QuadletUnitInfo[]>([])
...
const hasOverlayView = computed(() => route.path !== '/ship/volumes')

const sortedVolumes = computed(() => {
  return [...volumes.value].sort((a, b) => a.name.localeCompare(b.name))
})

const statusLabel = getQuadletStatusLabel
const statusClass = (item: QuadletUnitInfo) => getQuadletStatusToneClass(statusLabel(item))

async function loadVolumes(mode: 'initial' | 'refresh' = 'initial') {
  if (mode === 'initial') {
...
<template>
  <div class="relative flex flex-col gap-4 p-3 md:gap-6 md:p-6">
    <div class="flex flex-col gap-3 md:flex-row md:flex-wrap md:items-start md:justify-between md:gap-4">
      <div>
        ...
        <p class="mt-2 max-w-2xl text-sm text-neutral-500 dark:text-neutral-400">
          Control and manage your quadlet volume units.
        </p>
      </div>
    ...
    <section class="border border-neutral-200 bg-white/80 p-3 backdrop-blur-sm dark:border-neutral-800 dark:bg-neutral-950/60 md:p-4">
      <div class="flex items-center justify-between gap-3 border-b border-neutral-200 px-2 pb-4 dark:border-neutral-800">
        <div>
          <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
            {{ sortedVolumes.length }} volume{{ sortedVolumes.length === 1 ? '' : 's' }} found
          </p>
        </div>
      </div>
      ...
      <div v-else-if="!sortedVolumes.length" class="mt-3 flex min-h-40 flex-col items-center justify-center gap-3 border border-dashed border-neutral-200 bg-neutral-50/60 p-6 text-center dark:border-neutral-800 dark:bg-neutral-900/40 md:mt-4 md:min-h-64 md:p-8">
        <LucideDatabase class="size-10 text-neutral-300 dark:text-neutral-700" />
        <div>
          <p class="text-sm font-medium text-neutral-900 dark:text-white">
            No volumes yet
          </p>
          <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
            Create your first quadlet volume unit to get started.
          </p>
        </div>
      </div>
      ...
      <div v-else class="mt-3 flex flex-col gap-2 md:mt-4 md:gap-3">
        <NuxtLink
          ...
          :key="item.name"
          :to="`/ship/volumes/${encodeURIComponent(item.name)}`"
          ...
          :class="selectedVolumeName === item.name ? 'border-neutral-900 bg-neutral-50 dark:border-white dark:bg-neutral-900' : 'border-neutral-200 bg-white'"
        >
