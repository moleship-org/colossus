...
useHead(() => ({
  title: name.value ? `${name.value} network` : 'Network',
}))
...
async function loadNetwork() {
  ...
  try {
    const [fileContent, networkStatus] = await Promise.all([
      networks.read(name.value),
      networks.status(name.value),
    ])

    networkFile.value = fileContent
    status.value = JSON.parse(networkStatus)
  } catch (err) {
...
async function runOperation(operation: 'start' | 'stop' | 'restart' | 'delete') {
  ...
  try {
    if (operation === 'start') {
      await networks.start(name.value)
    } else if (operation === 'stop') {
      await networks.stop(name.value)
    } else if (operation === 'restart') {
      await networks.restart(name.value)
    } else {
      await networks.remove(name.value)
      await navigateTo('/ship/networks')
      return
    }
      ...
      <div class="grid gap-3 md:gap-4 lg:grid-cols-1">
        <section class="border border-neutral-200 bg-neutral-50/80 p-5 dark:border-neutral-800 dark:bg-neutral-900/60">
          <div class="flex flex-wrap items-center justify-start gap-3">
            <NuxtLink
              :to="`/ship/networks/edit/${encodeURIComponent(name)}`"
              class="btn-fill"
            >
        ...
        <section class="border border-neutral-200 bg-neutral-50/80 p-5 dark:border-neutral-800 dark:bg-neutral-900/60">
          ...
          <dl class="mt-4 grid gap-4 text-sm md:grid-cols-2">
            ...
            <div>
              ...
              <dd class="mt-1 font-medium text-neutral-900 dark:text-white">Network</dd>
            </div>
        ...
        <section class="border border-neutral-200 bg-neutral-50/80 p-5 dark:border-neutral-800 dark:bg-neutral-900/60">
          ...
          <pre class="mt-2 max-h-112 overflow-auto whitespace-pre-wrap wrap-break-word bg-white p-4 font-mono text-xs leading-relaxed text-neutral-700 dark:bg-neutral-950 dark:text-neutral-300">{{ isLoading ? 'Loading…' : networkFile }}</pre>
        </section>
      </div>
    </template>
  </div>
