...
useHead(() => ({
  title: originalName.value ? `Edit ${originalName.value}` : 'Edit network',
}))

async function loadDefaults() {
  if (!originalName.value) {
    errorMessage.value = 'Missing network name.'
  ...
  try {
    const content = await networks.read(originalName.value)
    const parsed = parseNetworkUnitFile(content, originalName.value)
...
async function handleSubmit() {
  ...
  try {
    ...
    await networks.remove(originalName.value)
    ...
    <form v-else class="flex flex-col gap-8" @submit.prevent="handleSubmit">
      <div class="grid gap-4 md:grid-cols-2">
        ...
        <label class="flex flex-col gap-2 text-sm">
          ...
          <input v-model="form.network_name" type="text" class="border border-neutral-200 bg-transparent px-3 py-2 text-neutral-900 outline-none focus:border-neutral-900 dark:border-neutral-800 dark:text-white dark:focus:border-white">
        </label>
        ...
        <label class="flex flex-col gap-2 text-sm">
          ...
          <input v-model="form.ipam_driver" type="text" class="border border-neutral-200 bg-transparent px-3 py-2 text-neutral-900 outline-none focus:border-neutral-900 dark:border-neutral-800 dark:text-white dark:focus:border-white">
        </label>

        <label class="flex flex-col gap-2 text-sm md:col-span-2">
          <span class="font-mono text-[11px] uppercase tracking-widest text-neutral-400 dark:text-neutral-600">Interface name</span>
          <input v-model="form.interface_name" type="text" class="border border-neutral-200 bg-transparent px-3 py-2 text-neutral-900 outline-none focus:border-neutral-900 dark:border-neutral-800 dark:text-white dark:focus:border-white">
        </label>
      </div>
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <label class="flex flex-col gap-2 text-sm">
          ...
          <textarea v-model="raw.subnet" rows="4" class="border border-neutral-200 bg-transparent p-3 font-mono text-sm text-neutral-900 outline-none focus:border-neutral-900 dark:border-neutral-800 dark:text-white dark:focus:border-white"></textarea>
        </label>

        <label class="flex flex-col gap-2 text-sm">
          <span class="font-mono text-[11px] uppercase tracking-widest text-neutral-400 dark:text-neutral-600">Gateway</span>
          <textarea v-model="raw.gateway" rows="4" class="border border-neutral-200 bg-transparent p-3 font-mono text-sm text-neutral-900 outline-none focus:border-neutral-900 dark:border-neutral-800 dark:text-white dark:focus:border-white"></textarea>
        </label>

        <label class="flex flex-col gap-2 text-sm">
          <span class="font-mono text-[11px] uppercase tracking-widest text-neutral-400 dark:text-neutral-600">IP range</span>
          <textarea v-model="raw.ip_range" rows="4" class="border border-neutral-200 bg-transparent p-3 font-mono text-sm text-neutral-900 outline-none focus:border-neutral-900 dark:border-neutral-800 dark:text-white dark:focus:border-white"></textarea>
        </label>

        <label class="flex flex-col gap-2 text-sm">
          <span class="font-mono text-[11px] uppercase tracking-widest text-neutral-400 dark:text-neutral-600">DNS</span>
          <textarea v-model="raw.dns" rows="4" class="border border-neutral-200 bg-transparent p-3 font-mono text-sm text-neutral-900 outline-none focus:border-neutral-900 dark:border-neutral-800 dark:text-white dark:focus:border-white"></textarea>
        </label>
        ...
        <label class="flex flex-col gap-2 text-sm">
          ...
          <textarea v-model="raw.labels" rows="4" class="border border-neutral-200 bg-transparent p-3 font-mono text-sm text-neutral-900 outline-none focus:border-neutral-900 dark:border-neutral-800 dark:text-white dark:focus:border-white"></textarea>
        </label>
