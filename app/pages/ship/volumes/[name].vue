...
const volumeFile = ref('')
const status = ref<VolumeStatus | null>(null)
const errorMessage = ref('')
const isLoading = ref(true)
const activeOperation = ref('')

useHead(() => ({
  title: name.value ? `${name.value} volume` : 'Volume',
}))
...
async function loadVolume() {
  ...
  try {
    const [fileContent, volumeStatus] = await Promise.all([
      volumes.read(name.value),
      volumes.status(name.value),
    ])

    volumeFile.value = fileContent
    status.value = JSON.parse(volumeStatus)
  } catch (err) {
...
async function runOperation(operation: 'start' | 'stop' | 'restart' | 'delete') {
  ...
  try {
    if (operation === 'start') {
      await volumes.start(name.value)
    } else if (operation === 'stop') {
      await volumes.stop(name.value)
    } else if (operation === 'restart') {
      await volumes.restart(name.value)
    } else {
      await volumes.remove(name.value)
      await navigateTo('/ship/volumes')
      return
    }
      ...
      <div class="grid gap-3 md:gap-4 lg:grid-cols-1">
        <section class="border border-neutral-200 bg-neutral-50/80 p-5 dark:border-neutral-800 dark:bg-neutral-900/60">
          <div class="flex flex-wrap items-center justify-start gap-3">
            <NuxtLink
              :to="`/ship/volumes/edit/${encodeURIComponent(name)}`"
              class="btn-fill"
            >
        ...
        <section class="border border-neutral-200 bg-neutral-50/80 p-5 dark:border-neutral-800 dark:bg-neutral-900/60">
          ...
          <dl class="mt-4 grid gap-4 text-sm md:grid-cols-2">
            ...
            <div>
              ...
              <dd class="mt-1 font-medium text-neutral-900 dark:text-white">Volume</dd>
            </div>
        ...
        <section class="border border-neutral-200 bg-neutral-50/80 p-5 dark:border-neutral-800 dark:bg-neutral-900/60">
          ...
          <pre class="mt-2 max-h-112 overflow-auto whitespace-pre-wrap wrap-break-word bg-white p-4 font-mono text-xs leading-relaxed text-neutral-700 dark:bg-neutral-950 dark:text-neutral-300">{{ isLoading ? 'Loading…' : volumeFile }}</pre>
        </section>
      </div>
    </template>
  </div>
